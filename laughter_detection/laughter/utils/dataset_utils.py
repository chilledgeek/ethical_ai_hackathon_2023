...
from utils import text_utils, audio_utils
...
def get_transcription_files(folder):
    return [
        f
        for f in librosa.util.find_files(folder, ext="text")
        if f.endswith("word.text")
    ]
...
def get_laughter_rows_from_file(f, include_words=False):
    if include_words:
        ...
        return [
            l for l in get_text_from_file(f) if "[laughter]" in l
...
def get_transcriptions_files(folder, a_or_b):
    ...
    for f in subfolders:
        fs = [
            ...
            if "a-word.text" in fname and a_or_b in fname
        ]
        files += fs
    files.sort()
    return files
...
def no_laughter_present(t_files, start, end):
    for t_file in t_files:
        ...
        for row in all_rows:
            ...
            if audio_utils.times_overlap(
                float(region_start), float(region_end), float(start), float(end)
            ):
...
def get_random_speech_region_from_files(
    t_files, audio_length, region_length, random_seed=None
):
...
def get_laughter_regions_and_speech_regions(
    text_A, text_B, audio_file, random_seed=None, include_words=False
):
    ...
    for laughter_region in laughter_regions:
        ...
        speech_regions.append(
            get_random_speech_region_from_files(
    ...
    laughter_regions = [
        audio_utils.start_end_to_offset_duration(s, e) for s, e in laughter_regions
    ]
    speech_regions = [
        audio_utils.start_end_to_offset_duration(s, e) for s, e in speech_regions
    ]
...
def get_laughter_speech_text_lines(
    ...
    (
        ...
        laughter_region_subsamples,
        speech_region_subsamples,
    ) = get_laughter_regions_and_speech_regions(
